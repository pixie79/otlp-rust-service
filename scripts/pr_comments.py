#!/usr/bin/env python3
"""
Manage comments on a GitHub Pull Request.

This script can:
- Fetch all types of comments from a PR (issue comments, review comments, reviews)
- Reply to individual comments
- Update multiple comments from a JSON file

Usage:
    python pr_comments.py <owner> <repo> <pr_number> [options]
    python pr_comments.py <pr_url> [options]

Examples:
    # Fetch comments
    python pr_comments.py pixie79 otlp-rust-service 123
    python pr_comments.py https://github.com/pixie79/otlp-rust-service/pull/123 --json
    
    # Update comments from JSON file
    python pr_comments.py pixie79 otlp-rust-service 12 --update-comment pr_comments_response.json --token ghp_xxxxx
    
    # Reply to a single comment
    python pr_comments.py pixie79 otlp-rust-service 123 --reply 12345 --comment-type review_comment --reply-body "Fixed!" --token ghp_xxxxx
...
class GitHubPRComments:
    ...
    def __init__(self, owner: str, repo: str, token: Optional[str] = None):
        ...
        self.headers = {
            ...
            "User-Agent": "PR-Comments-Manager",
        }
        if self.token:
    ...
    def update_comments_from_json(self, pr_number: int, json_file: str, comment_type: str = "review_comment") -> Dict:
        """
        Update multiple comments from a JSON file.
        ...
        Args:
            pr_number: Pull request number
            json_file: Path to JSON file containing comments to update
            comment_type: Type of comments to update (default: "review_comment")
            
        Returns:
            Dictionary with summary of updates
        """
        if not self.token:
            print("Error: GitHub token is required for updating comments", file=sys.stderr)
            sys.exit(1)
        ...
        if "comments" not in data:
            print("Error: JSON file must have a 'comments' array", file=sys.stderr)
            sys.exit(1)
        
        comments = data.get("comments", [])
        if not comments:
            print("Warning: No comments found in JSON file", file=sys.stderr)
            return {"success": 0, "failed": 0, "total": 0}
        
        print(f"Updating {len(comments)} comments from {json_file}...", file=sys.stderr)
        ...
        for i, comment_data in enumerate(comments, 1):
            ...
            if not comment_id:
                print(f"Warning: Comment {i} missing 'comment_id', skipping", file=sys.stderr)
                results["failed"] += 1
                results["errors"].append(f"Comment {i}: Missing comment_id")
                continue
            
            if not body:
                print(f"Warning: Comment {i} (ID: {comment_id}) missing 'body', skipping", file=sys.stderr)
                results["failed"] += 1
                results["errors"].append(f"Comment {comment_id}: Missing body")
                continue
            
            try:
                result = self.reply_to_comment(pr_number, comment_id, comment_type, body)
                print(f"✓ Comment {i}/{len(comments)} (ID: {comment_id}) updated successfully", file=sys.stderr)
                print(f"  URL: {result.get('html_url', 'N/A')}", file=sys.stderr)
                results["success"] += 1
                ...
                # Error already printed by reply_to_comment
                results["failed"] += 1
                results["errors"].append(f"Comment {comment_id}: Failed to update")
            except Exception as e:
                print(f"Error updating comment {comment_id}: {e}", file=sys.stderr)
                results["failed"] += 1
                results["errors"].append(f"Comment {comment_id}: {str(e)}")
        
        print(f"\nSummary: {results['success']} succeeded, {results['failed']} failed out of {results['total']} total", file=sys.stderr)
        return results
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Manage comments on a GitHub Pull Request",
    ...
    parser.add_argument(
        ...
        help="Comment type for --detail, --reply, or --update-comment (required with these options)",
    )
    parser.add_argument(
        "--reply",
        type=int,
    ...
    parser.add_argument(
        "--update-comment",
        metavar="JSON_FILE",
        help="Update multiple comments from a JSON file (see pr_comments_response.json for format)",
    )
    parser.add_argument(
        "--status",
    ...
    # Handle update comments from JSON file
    if args.update_comment:
        if not args.comment_type:
            # Default to review_comment if not specified
            comment_type = "review_comment"
            print("Using default comment type: review_comment", file=sys.stderr)
        else:
            comment_type = args.comment_type
        
        if not args.token:
            print("Error: --token is required for --update-comment", file=sys.stderr)
            sys.exit(1)
        
        results = client.update_comments_from_json(pr_number, args.update_comment, comment_type)
    ...
    if args.detail:
        print("Error: --detail option is not yet implemented", file=sys.stderr)
        sys.exit(1)
