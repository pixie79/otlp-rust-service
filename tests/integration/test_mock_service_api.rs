//! Integration test for mock service public API interface
...
#[tokio::test]
async fn test_mock_service_api_trace() {
...
#[tokio::test]
async fn test_mock_service_api_metrics() {
    let service = MockOtlpService::new();
...
#[tokio::test]
async fn test_mock_service_api_multiple_traces() {
    let service = MockOtlpService::new();
    ...
    // Verify all traces were received
    assert!(service.assert_traces_received(5).await.is_ok());
    assert_eq!(service.api_calls_count().await, 5);
}

#[tokio::test]
async fn test_mock_service_api_reset() {
    let service = MockOtlpService::new();
