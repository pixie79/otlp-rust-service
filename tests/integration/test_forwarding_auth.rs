...
#[tokio::test]
async fn test_forwarding_with_api_key_auth() {
    ...
    // Export span - forwarding will fail but should use auth headers
...
#[tokio::test]
async fn test_forwarding_with_bearer_token_auth() {
    let temp_dir = TempDir::new().unwrap();
...
#[tokio::test]
async fn test_forwarding_with_basic_auth() {
    let temp_dir = TempDir::new().unwrap();
