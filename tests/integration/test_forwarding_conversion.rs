...
#[tokio::test]
async fn test_forwarding_protobuf_input_arrow_flight_output() {
    let temp_dir = TempDir::new().unwrap();
    
    // Configure forwarding with Arrow Flight protocol (input is Protobuf via gRPC, output is Arrow Flight)
    let forwarding = ForwardingConfig {
        enabled: true,
        endpoint_url: Some("http://localhost:9999".to_string()),
        protocol: ForwardingProtocol::ArrowFlight, // Output format
        authentication: None,
    };
    ...
    // Create a test span (this simulates Protobuf input)
    ...
    // Export span - should convert Protobuf to Arrow Flight for forwarding
...
#[tokio::test]
async fn test_forwarding_arrow_flight_input_protobuf_output() {
    let temp_dir = TempDir::new().unwrap();
    
    // Configure forwarding with Protobuf protocol (input is Arrow Flight via gRPC, output is Protobuf)
    let forwarding = ForwardingConfig {
        enabled: true,
        endpoint_url: Some("http://localhost:9999".to_string()),
        protocol: ForwardingProtocol::Protobuf, // Output format
        authentication: None,
    };
    ...
    // Create a test span (this simulates Arrow Flight input)
    ...
    let span = SpanData {
        ...
        parent_span_id: SpanId::INVALID,
        span_kind: SpanKind::Internal,
        name: std::borrow::Cow::Borrowed("test-span-arrow"),
        start_time: std::time::SystemTime::now(),
        end_time: std::time::SystemTime::now(),
        attributes: vec![].into_iter().collect(),
        events: opentelemetry_sdk::trace::SpanEvents::default(),
    ...
    // Export span - should convert Arrow Flight to Protobuf for forwarding
    library.export_trace(span).await.unwrap();
