...
fn test_dashboard_config_validation_invalid_bind_address() {
    ...
    let config = DashboardConfig {
        ...
        port: 8080,
        static_dir: temp_dir.path().to_path_buf(),
        bind_address: "invalid-ip-address".to_string(),
    };

    let result = config.validate();
    assert!(result.is_err());
    if let Err(OtlpConfigError::ValidationFailed(msg)) = result {
        assert!(msg.contains("bind_address must be a valid IP address"));
    } else {
        panic!("Expected ValidationFailed error");
    }
}
...
#[test]
fn test_dashboard_config_validation_valid_bind_addresses() {
    let temp_dir = tempfile::tempdir().unwrap();
    
    // Test valid IPv4 addresses
    for addr in ["127.0.0.1", "0.0.0.0", "192.168.1.1"] {
        let config = DashboardConfig {
            enabled: true,
            port: 8080,
            static_dir: temp_dir.path().to_path_buf(),
        ...
        assert!(config.validate().is_ok(), "Failed for address: {}", addr);
    }
}
