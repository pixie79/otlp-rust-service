...
2. **Code Quality Checks**: The following commands MUST pass without errors or warnings for ALL languages in the codebase:
   ...
   - **JavaScript/TypeScript code** (for dashboard and other JS/TS code):
     ...
     - `npm test` MUST pass (all JavaScript/TypeScript unit tests)
     - `npm run test:e2e` MUST pass (all E2E tests if applicable)
   - **CSS/SCSS code**:
     - `npm run format:check` or `prettier --check` MUST pass (CSS formatting)
     - CSS linting MUST pass (if configured)
   - **Other languages** (YAML, JSON, Markdown, etc.):
     - `prettier --check` MUST pass for all supported file types
     - Language-specific linters MUST pass if configured
...
**CSS/SCSS Code:**
- `npm run format:check` or `prettier --check "dashboard/**/*.css"` MUST pass (CSS formatting)
- CSS linting MUST pass (if stylelint or similar is configured)

**All Other Languages:**
- `prettier --check` MUST pass for all supported file types (YAML, JSON, Markdown, HTML, etc.)
- Language-specific linters MUST pass if configured
