...
export class TraceList {
  ...
  _applyFilters() {
    ...
    this.filteredTraces = this.traces.filter((trace) => {
      ...
      if (filters.serviceName && !trace.serviceName?.toLowerCase().includes(filters.serviceName.toLowerCase())) {
        return false;
      }
  ...
  _renderRow(trace, index) {
    ...
    const startTime = typeof trace.startTime === 'number' ? trace.startTime : Date.now() * 1_000_000;
