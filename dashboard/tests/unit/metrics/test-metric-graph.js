...
vi.mock('plotly.js-dist-min', () => ({
  default: {
    react: vi.fn((container, traces, layout, config) => Promise.resolve()),
    extendTraces: vi.fn((container, updateData, indices, maxPoints) => {}),
    purge: vi.fn((container) => {}),
  },
}));
...
describe('MetricGraph', () => {
  ...
  it('uses extendTraces for efficient updates when graph already rendered', async () => {
    ...
    const newMetrics = [buildMetric({ value: 200, timestamp: Date.now() * 1_000_000 + 10_000_000 })];
